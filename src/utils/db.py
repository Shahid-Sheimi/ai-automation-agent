...
def get_db_session():
    ...
    """
    interactions_db_url = f"{settings.database_url}"
...
def get_interactions_from_db(limit: int = 5):
    ...
    try:
        ...
        with SessionLocal() as session:
            ...
            print("interactionsasdfkasdjkf.....",interactions)
...
def search_qdrant(
    ...
    client_connection = QdrantClient(
        ...
        api_key=getattr(settings.qdrant, "api_key", ""),
    )
    ...
    try:
        if query_vector is None:
            if embedding_model is not None:
                ...
                logging.warning("No query vector available for Qdrant search.")
                return {"answer": "Unable to process the query at the moment."}
        elif not isinstance(query_vector, list):
            query_vector = list(query_vector)

        search_response = client_connection.query_points(
        ...
        search_results = getattr(search_response, "points", [])
